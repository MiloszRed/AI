...
namespace projekt1
{
    public class Schedule
    {
        ...
        public static Schedule GenerateWithOrderedJobs(List<Job> jobs, int processorsCount)
        {
            var schedule = new Schedule(processorsCount);
            foreach (var job in jobs)
            {
                schedule.AssignJobsInOrder(job);
            }
            return schedule;
        }

        private void AssignJobsInOrder(Job job)
        {
            Job jobTemp = job;
            Processor processor = Processors.OrderBy(p => p.EndTime()).FirstOrDefault();

            while (jobTemp != null)
            {
                processor.AddJob(jobTemp);
        ...
        // kopiowanie harmonogramu i jego listy procesorów
